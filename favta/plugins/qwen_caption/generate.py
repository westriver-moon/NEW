...
5. Keep each paraphrase at most {max_words} words.
...
def generate_batch(model, tokenizer, torch, captions: Sequence[str], args, attempt: int) -> List[str]:
    prompts = [render_chat(tokenizer, caption, args.max_words) for caption in captions]
    inputs = tokenizer(
        prompts,
        return_tensors="pt",
        padding=True,
    ...
    generator.manual_seed(args.seed + args.shard_id * 1_000_003 + attempt)
    ...
    try:
        for batch_start in range(0, len(pending), args.batch_size):
            ...
            if processed % args.checkpoint_every < args.batch_size or processed == len(pending):
                ...
                materialize(output_path, manifest_path, source, completed, metadata)
                if failures:
                    atomic_json(output_dir / f"failures.{tag}.json", failures)
